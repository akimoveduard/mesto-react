...
function Card({card, onCardClick, onCardLike}) {
  ...
  return (
    <li className="card">
      <button className={`button button_type_delete ${cardDeleteButtonClassName}`} type="button"></button>
      <img className="card__image" src={card.link} alt={card.name} onClick={handleCardClick} />
      <div className="card__info">
        <h2 className="card__caption">{card.name}</h2>
        <figure className="card__likes">
          <button className={`button ${cardLikeButtonClassName}`} type="button" onClick={handleCardLike}></button>
